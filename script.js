...
function makePool(src) {
  ...
  return function play() {
    ...
    a.currentTime = 0;
    const p = a.play();
    if (p) p.catch(err => console.warn('[steve] playback blocked:', err.name));
  };
}
...
const CANDY = ['#e8352c', '#f5821f', '#f7c815', '#58b947', '#ffd9a0', '#9b59d0'];
...
console.log('[steve] ready. odds 1 in 15.' + (forceGummy ? ' (?gummy=1 — rigged)' : ''));
